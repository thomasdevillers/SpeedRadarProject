...
export function ReleaseUploadForm() {
  ...
  async function submit(formData: FormData) {
    ...
    if (process.env.NEXT_PUBLIC_DEMO_MODE === "true") { setMessage("Signed release registered"); router.refresh(); return; }
...
export function RolloutForm({ devices, releases }: { devices: DeviceSummary[]; releases: ReleaseSummary[] }) {
  ...
  async function submit(formData: FormData) {
    ...
    if (process.env.NEXT_PUBLIC_DEMO_MODE === "true") { setMessage("Canary rollout scheduled"); router.refresh(); return; }
