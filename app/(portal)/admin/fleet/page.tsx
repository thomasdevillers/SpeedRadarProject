...
import { getDashboardData, getDeviceAssignments, getOrganizations } from "@/lib/portal-data";

export const metadata = { title: "Fleet control" };

export default async function FleetPage() {
  const [{ devices }, organizations, assignments] = await Promise.all([getDashboardData(), getOrganizations(), getDeviceAssignments()]);
  return (
    <>
      ...
      <section className="panel reveal section-block"><div className="panel-head"><div><span className="eyebrow">Rental control</span><h2>Assign a radar</h2></div></div><AssignmentForm organizations={organizations} devices={devices} assignments={assignments} /></section>
